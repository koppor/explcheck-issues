...
% The softpath mechanism detaches the drawing commands from the actual drawing and creates another layer of
% commands that can be used to manipulate the output of the original drawing commands. This enables it to add
% functionality such as shortening or offsetting paths. It also allows for other functions that are unrelated
% to drawing such as measuring the length of a path.
...
\cs_new_protected:Npn \hawkdraw_softpath_rectangle_op:nnNnn #1#2#3#4#5 {
    \draw_path_rectangle_corners:nn { #1 , #2 } { #4 , #5 }
}
...
\cs_new_protected:Npn \hawkdraw_softpath_shorten_moveto_op:nn #1#2 {
    ...
    \bool_lazy_and:nnF {
        \dim_compare_p:nNn { \l_hawkdraw_softpath_shorten_start_dim } > { \c_zero_dim }
    } {
...
\cs_new_protected:Npn \__hawkdraw_softpath_shorten_line:nn #1#2 {
    ...
    \bool_lazy_and:nnTF {
        \dim_compare_p:nNn { \l_hawkdraw_softpath_shorten_end_dim } > { \c_zero_dim }
    } {
...
\cs_new_protected:Npn \hawkdraw_softpath_offset_construct_cubic_set_split:Nnnnnn #1#2#3#4#5#6 {
    ...
    \int_compare:nNnT { \seq_count:N #1 } > { \c_one_int } {
