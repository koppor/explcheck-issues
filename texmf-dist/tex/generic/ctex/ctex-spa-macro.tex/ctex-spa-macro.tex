...
\cs_new_protected:Npn \__ctex_write_family:nn #1#2
  {
    \group_begin:
      ...
      \seq_map_inline:Nn \c__ctex_punct_seq
        {
          \exp_args:No \__ctex_save_bounds:n
            { \int_use:N \tex_XeTeXcharglyph:D ##1 }
        }
      \iow_now:Ne \g__ctex_spa_iow
        {
...
\prg_new_protected_conditional:Nnn \__ctex_if_font_exists:n { TF }
  {
    \tl_clear:N \l__ctex_if_otf_exists_tl
    \tl_clear:N \l__ctex_if_ttf_exists_tl
    \bool_case:nF
      {
        { \file_if_exist_p:n { #1.otf }          }
        { \tl_set:Nn \l__ctex_if_otf_exists_tl {#1} }
        { \file_if_exist_p:n { #1.ttf }          }
        { \tl_set:Nn \l__ctex_if_ttf_exists_tl {#1} }
      }
      {
        \sys_get_shell:nnN { kpsewhich ~ #1.otf } { \endlinechar=-1 }
          \l__ctex_if_otf_exists_tl
        \sys_get_shell:nnN { kpsewhich ~ #1.ttf } { \endlinechar=-1 }
          \l__ctex_if_ttf_exists_tl
      }
    \bool_lazy_and:nnTF
      { \tl_if_blank_p:V \l__ctex_if_otf_exists_tl }
      { \tl_if_blank_p:V \l__ctex_if_ttf_exists_tl }
      { \prg_return_false: } { \prg_return_true: }
  }
\tl_clear_new:N \l__ctex_if_otf_exists_tl
\tl_clear_new:N \l__ctex_if_ttf_exists_tl
\cs_new_protected:Npn \__ctex_save_bounds:n #1
  {
    \clist_put_right:Ne \l__ctex_punct_bounds_clist
      {
