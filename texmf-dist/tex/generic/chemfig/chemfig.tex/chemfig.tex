...
\def\CF_chemfig_c#1{% #1 est le code de la molécule
	...
	\fi
%	\showtokens\expandafter{\expanded {Dangle=<\CF_defaultangle>,Laction=\CF_lastaction, Acorr=\unexpanded\expandafter{\CF_cycleanglecorrection},cntcycle=\the\CF_cnt_cyclebonds },mol=<#1>}%
...
\def\CF_drawatomgroup#1#2#3{% #1=angle d'arrivée de la liaison #2=numero atome sur lequel arrive la liaison #3=groupe d'atomes
	...
	\repeat
	\CF_cnt_atom\CF_hookatomnumber
	\ifnum\CF_lastaction=2 % s'il faut tracer une liaison alors c'est qu'un cycle ne peut pas commencer la molécule : annulation de la correction d'angle
		\def\CF_cycleanglecorrection{0}% Bugfix 1.82 \def au lieu de \gdef
